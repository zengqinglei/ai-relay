...
@Component({
  ...
  template: `
    <div
      class="w-full h-full"
      [ngClass]="{
        'text-blue-600 dark:text-blue-400': currentKind() === 'Gemini' || currentKind() === 'Google',
        'text-orange-600 dark:text-orange-500': currentKind() === 'Claude' || currentKind() === 'Anthropic',
    ...
    >
      @if (currentKind() === 'Gemini' || currentKind() === 'Google') {
        <svg viewBox="0 0 24 24" fill="currentColor" class="w-full h-full">
          <path
            d="M11.04 19.32Q12 21.51 12 24q0-2.49.93-4.68.96-2.19 2.58-3.81t3.81-2.55Q21.51 12 24 12q-2.49 0-4.68-.93a12.3 12.3 0 0 1-3.81-2.58 12.3 12.3 0 0 1-2.58-3.81Q12 2.49 12 0q0 2.49-.96 4.68-.93 2.19-2.55 3.81a12.3 12.3 0 0 1-3.81 2.58Q2.49 12 0 12q2.49 0 4.68.96 2.19.93 3.81 2.55t2.55 3.81"
          />
        </svg>
      } @else if (currentKind() === 'Claude' || currentKind() === 'Anthropic') {
        <svg viewBox="0 0 24 24" fill="currentColor" class="w-full h-full">
          <path
      ...
      } @else if (currentKind() === 'Antigravity') {
        <!-- Antigravity 图标：火箭/上升箭头 -->
        <svg viewBox="0 0 24 24" fill="currentColor" class="w-full h-full">
          <!-- 主体火箭 -->
...
export class PlatformIcon {
  ...
  vendor = input<ModelVendor | null>(null);

  currentKind(): Provider | ModelVendor | null {
    return this.provider() ?? this.vendor();
  }
}

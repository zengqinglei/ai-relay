...
public class UsageRecordMetricAppService(
    ...
    ICurrentUser currentUser) : BaseAppService, IUsageRecordMetricAppService
{
    ...
    {
        var query = await GetUserQueryAsync(cancellationToken);
        ...
        // 2. 上一周期总请求 (用于计算趋势)
        var previousRequests = await asyncExecuter.CountAsync(
            query.Where(r => r.CreationTime >= previousPeriodStart && r.CreationTime < previousPeriodEnd),
            cancellationToken);
        ...
        var requestsLastMinute = await asyncExecuter.CountAsync(
            query.Where(r => r.CreationTime >= oneMinuteAgo),
            cancellationToken);
    ...
    public async Task<List<ApiKeyTrendOutputDto>> GetTopApiKeyTrendAsync(DateTime? startTime = null, DateTime? endTime = null, CancellationToken cancellationToken = default)
    {
        ...
        var trendData = await asyncExecuter.ToListAsync(query
            .Where(r => r.CreationTime >= start && r.CreationTime < end && r.ApiKeyName != null && topKeyNames.Contains(r.ApiKeyName))
    ...
    private async Task<IQueryable<UsageRecord>> GetUserQueryAsync(CancellationToken cancellationToken)
    {

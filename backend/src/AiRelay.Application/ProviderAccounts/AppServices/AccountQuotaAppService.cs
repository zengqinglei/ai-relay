    ...
    ILogger<AccountQuotaAppService> logger) : BaseAppService, IAccountQuotaAppService
{
    ...
    public async Task RefreshAllQuotasAsync(CancellationToken cancellationToken = default)
    {
        ...
        accountQuery = accountQuery.Where(a => 
            ((a.Provider == Provider.Antigravity && a.AuthMethod == AuthMethod.OAuth) || 
             (a.Provider == Provider.Gemini && a.AuthMethod == AuthMethod.OAuth)) && 
            a.IsActive);
        ...
        foreach (var accountToken in accountTokens)
        {
            try
            {
                var client = clientFactory.CreateHandler(accountToken.Provider, accountToken.AuthMethod, accountToken.AccessToken!, accountToken.BaseUrl, accountToken.ExtraProperties);
                ...
                foreach (var quotaInfo in quotaInfos)
                {
                    ...
                    await cache.SetStringAsync(
                        ...
                        cancellationToken);
                    logger.LogDebug(
                        "账户 {AccountName} ({Provider} {AuthMethod}) 配额详情: Model={Model}, Remaining={Remaining}, Tier={Tier}",
                        accountToken.Name,
            ...
            {
                failureCount++;
                logger.LogError(ex, "刷新账户 {AccountName} ({Provider}-{AuthMethod}) 配额时发生错误", accountToken.Name, accountToken.Provider, accountToken.AuthMethod);
            }
        }
